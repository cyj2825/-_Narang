...
> OAuth 2.0은 인터넷 사용자가 다른 웹 사이트 또는 애플리케이션에 자신의 리소스에 대한 제한된 액세스 권한을 부여할 수 있게 하는 프로토콜입니다.

### 적용
`나랑` 에서는 사용자 인증 및 권한 부여를 위해 OAuth 2.0을 사용했습니다. 이를 통해 사용자는 다른 서비스(예: 카카오, 네이버) 계정으로 로그인하고, 애플리케이션에서 필요한 닉네임, 성별, 연령대 등을 얻을 수 있습니다.


## JWT (JSON Web Token)
<aside>

> JWT는 정보를 안전하게 전송하기 위한 간결하고 독립적인 방법을 제공하는 개방형 표준입니다. 주로 사용자의 인증 및 정보 교환에 활용됩니다.

### 적용
`나랑` 에서는 JWT를 사용하여 사용자 세션을 관리하고, 정보를 안전하게 전달하는 데 활용했습니다. 토큰의 발급, 검증, 갱신 등이 구현되어 있습니다.
